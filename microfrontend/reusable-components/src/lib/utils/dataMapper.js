...
export const getKeyValue = (src, keyStr) => {
...
const select_arr = (src, key, keys) => {
  ...
  if (typeof data[0] !== 'undefined' && key.name && data[0][key.name]) return data[0][key.name]
...
const select_obj_keys = (src, keys) => {
  let data = []
  let n = 0
...
export const setKeyValue = (dest, keyStr, data, context = {}) => {
  ...
  if (Array.isArray(keyStr)) {
    for (let i = 0; i < keyStr.length; i++) {
      // The substring value is in string notation - recurs with the key string
      if (typeof keyStr[i] == 'string') dest = setKeyValue(dest, keyStr[i], data, context)
      // The substring value is in array notation - recurs with the key from the array
  ...
  else if (typeof keyStr == 'string') dest = update(dest, data, parse(keyStr), context)
...
const update = (dest, data, keys, context) => {
  if (keys) {
...
const update_obj = (dest, key, data, keys, context) => {
...
const update_arr = (dest, key, data, keys, context) => {
...
const applyTransform = (data, dest, context) => {
...
const update_arr_ix = (dest, ix, data, keys, context) => {
  let o
...
const set_data = (dest, key, data, context) => {
  ...
  if (typeof data !== 'undefined' && key && key.name) {
    ...
    if (
      ...
      if (key && key.name[0] == '$') {
        dest[dest[key.name.slice(1)]] = data
      } else {
        dest[key.name] = data
      }
    }
...
export const parse = (key_str, delimiter = '.') => {
