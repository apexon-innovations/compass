...
def bitbucket_stats(event, context):
        ...
        try:
            ...
            if "commits" in _hygieiaPullRequest:
                for _commit in _hygieiaPullRequest["commits"]:
                    ...
                    scmAuthor = _commit["scmAuthor"]
                    prCommit["authorEmail"] = scmAuthor[scmAuthor.find(email_splitter_start)+len(email_splitter_start):scmAuthor.rfind(email_splitter_end)]
                    prCommit["authorUserId"] = scmAuthor[0:scmAuthor.rfind(email_splitter_start)].strip()
