...
public class BitbucketUrlParser {
	...
	/**
	 * This method constructs Bitbucket url parts from bitbucket url. Usefulness of this
	 * method is observed at the time of creating Bitbucket API endpoints.
	 * @param rawUrl
	...
	public static BitbucketUrlParts parseBitbucketUrl(String rawUrl) {
		...
		if (repoUrlProcessed.endsWith(GIT_EXTENSION)) {
			repoUrlProcessed = StringUtils.substring(repoUrlProcessed, 0,
					StringUtils.lastIndexOf(repoUrlProcessed, GIT_EXTENSION));
		}
	...
	private static String parseURLPath(URI uri) {
		...
		if ((path.startsWith(SCM_SUFFIXED_WITH_FORWARD_SLASH) || path.startsWith(SCM_PREFIXED_WITH_FORWARD_SLASH))
				&& path.length() > 4) {
	...
	public static class BitbucketUrlParts {
		...
		public BitbucketUrlParts(String scheme, String host, int port, String projectKey, String repositorySlug) {
			this.scheme = scheme;
			this.host = host;
			this.port = port;
