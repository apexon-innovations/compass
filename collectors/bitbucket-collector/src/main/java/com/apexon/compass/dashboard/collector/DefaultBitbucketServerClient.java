...
/**
 * Implementation of a git client to connect to an Atlassian Bitbucket <i>Server</i>
 * product.
 * <p>
 ...
 * https://confluence.atlassian.com/bitbucket/version-2-423626329.html</li>
 * <li><b>Bitbucket Server:</b>
 * https://developer.atlassian.com/static/rest/stash/3.11.3/stash-rest.html</li>
 * </ul>
 * <b>
 *
 * @see <a href=
...
public class DefaultBitbucketServerClient implements GitClient {
	...
	@Autowired
	public DefaultBitbucketServerClient(GitSettings settings, BitbucketApiUrlBuilder bitbucketApiUrlBuilder,
			SCMHttpRestClient scmHttpRestClient) {
	...
	public List<Commit> getCommits(GitRepo repo, boolean firstRun, String userName, String password) {
		...
		try {
			...
			URI queryUri = buildUri((String) repo.getOptions().get("url"), repo.getBranch(),
					repo.getLastUpdateCommit());
			...
			while (!lastPage) {
				...
				for (Object item : jsonArray) {
					...
					String author = JSONParserUtils.str(authorObject, "name");
					long timestamp = Long.valueOf(JSONParserUtils.str(jsonObject, "authorTimestamp"));
				...
				PageMetadata pageMetadata = new PageMetadata(queryUri, jsonParentObject, jsonArray, "server");
	...
	// package for junit
	@SuppressWarnings({ "PMD.NPathComplexity" })
	/* package */ URI buildUri(final String rawUrl, final String branch, final String lastKnownCommit)
