...
/**
 * This class represents metadata of a paginated response. It nicely encapsulates Page
 * Metadata by keeping track of last page and next page url.
 */
public class PageMetadata {
	...
	public PageMetadata(URI currentPageUrl, JSONObject jsonArray, JSONArray values, String productType) {
		...
		if (CollectionUtils.isEmpty(this.values)) {
			...
			if ("cloud".equals(productType)) {
				...
				if (nextPageStart != null && !"null".equals(nextPageStart)) {
					try {
						this.nextPageUrl = new URIBuilder(this.currentPageUrl).addParameter("start", nextPageStart)
							.build();
					}
					catch (URISyntaxException e) {
						throw new RuntimeException("Unable to create next page URI", e);
