...
public class DefaultBitbucketServerClientTest {
	...
	private static final String MYREPOSITORY_GIT = "https://username@company.com/scm/myproject/myrepository.git";
  ...
  public void init() throws URISyntaxException {
	...
	public void testGetCommits() throws IOException, URISyntaxException {
		...
		URI uri2 = URI.create(
				"https://company.com/rest/api/1.0/projects/myproject/repos/myrepository/commits?until=refs%2Fheads%2Fmaster&limit=1&start=1");

		when(rest.exchange(eq(uri1), eq(HttpMethod.GET), any(HttpEntity.class), eq(String.class)))
			.thenReturn(new ResponseEntity<>(jsonResponse1, HttpStatus.OK));
		when(rest.exchange(eq(uri2), eq(HttpMethod.GET), any(HttpEntity.class), eq(String.class)))
			.thenReturn(new ResponseEntity<>(jsonResponse2, HttpStatus.OK));

		when(scmHttpRestClient.makeRestCall(uri1, "abcdefgh", "abcdefghijklmnopqrstuvwxyz1234567"))
			.thenReturn(new ResponseEntity<>(jsonResponse1, HttpStatus.OK));
		when(scmHttpRestClient.makeRestCall(uri2, "abcdefgh", "abcdefghijklmnopqrstuvwxyz1234567"))
			.thenReturn(new ResponseEntity<>(jsonResponse2, HttpStatus.OK));
		...
		/*
		 * assertTrue(0 != commits.get(1).getTimestamp()); assertEquals(repoUrl,
		 * commits.get(1).getScmUrl());
		 * assertEquals("30a9559513e471fb8f1deff10bd8823ad74a2fab",
		 * commits.get(1).getScmRevisionNumber()); assertEquals("billybob",
		 * commits.get(1).getScmAuthor()); assertEquals("Message 2",
		 * commits.get(1).getScmCommitLog()); assertEquals(1,
		 * commits.get(1).getScmParentRevisionNumbers().size());
		 * assertEquals("9097aee6916a1883945b9cf9b77d351dc6802307",
		 * commits.get(1).getScmParentRevisionNumbers().get(0));
	...
	public void testBuildUri() throws URISyntaxException {
		...
		assertEquals(
				"https://company.com/rest/api/1.0/projects/myproject/repos/myrepository/commits?until=master&limit=25",
				res.toString());

		res = client.buildUri(url2, null, null);
		assertEquals(
				"https://company.com/rest/api/1.0/projects/space%20space/repos/myrepository/commits?until=master&limit=25",
				res.toString());

		res = client.buildUri(url3, null, null);
		assertEquals(
				"http://company.com/rest/api/1.0/projects/~myusername/repos/myrepository/commits?until=master&limit=25",
				res.toString());

		// try branch
		res = client.buildUri(url2, branch1, null);
		assertEquals(
				"https://company.com/rest/api/1.0/projects/space%20space/repos/myrepository/commits?until=refs%2Fheads%2Ffeature%2FNEW_STUFF_NOW&limit=25",
				res.toString());

		// try since
		res = client.buildUri(url2, null, lastKnownCommit1);
		assertEquals(
				"https://company.com/rest/api/1.0/projects/space%20space/repos/myrepository/commits?until=master&since=ABCDEFGHIJKLMNOPQRSTUVWXYZ&limit=25",
				res.toString());
		...
		assertEquals(
				"https://company.com/rest/api/1.0/projects/space%20space/repos/myrepository/commits?until=refs%2Fheads%2Fmaster",
				res.toString());
	}
