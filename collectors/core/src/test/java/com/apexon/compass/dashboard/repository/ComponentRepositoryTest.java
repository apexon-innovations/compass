...
class ComponentRepositoryTest {
	...
	@Test
	void findByCollectorTypeAndItemIdInReturns0() throws IOException {
		BooleanBuilder builder = new BooleanBuilder();
		PathBuilder<Component> path = new PathBuilder<>(Component.class, "components");
		builder.and(path.get("collectorItems", Map.class)
			.get(CollectorType.SCM.toString(), List.class)
			.get("_id", ObjectId.class)
			.in(Lists.newArrayList(new ObjectId("5ba136220be2d32568777fa5"))));
		...
		// List<Component> itemListResult =
		// componentRepository.findByCollectorTypeAndItemIdIn(CollectorType.SCM,
		// Lists.newArrayList(new ObjectId("5ba136220be2d32568777fa5")));
		assertThat(itemListResult).size().isEqualTo(0);
	}

	@Test
	void findByCollectorTypeAndItemIdInReturns3() throws IOException {
		doNothing().when(componentRepository).deleteAll();
		...
		when(componentRepository.findByCollectorTypeAndItemIdIn(CollectorType.Build,
				Lists.newArrayList(new ObjectId("5ba136220be2d32568777fa5"))))
			.thenReturn(itemList);
