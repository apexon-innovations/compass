...
class TestResultEventListenerTest {

	// Mockito.mock is required rather than previous iteration @Mock of parameter to
	// assure Interface is mocked
	...
	void test_testResultListener() {
		...
		when(collectorItemRepository.findById(any(ObjectId.class)))
			.thenReturn(Optional.of(getTestResultCollectorItem()));
		when(collectorRepository.save(any(Collector.class))).thenReturn(getPerfToolsCollector());
		when(collectorItemRepository.save(any(CollectorItem.class)))
			.thenReturn(getNewPerfCollectorItem(getPerfToolsCollector()));
		...
		when(testResultEventListener.createPerformanceDoc(testResult, getTestCapabilities().iterator().next(),
				perfCollItem))
