...
class RelatedCollectorItemRepositoryTest {
	...
	void saveRelatedItems() {
		...
		// List<RelatedCollectorItem> relatedCollectorItemList =
		// Lists.newArrayList(relatedCollectorItemRepository.findAll());
	...
	@Test
	void saveRelatedItemsDuplicate() {
		...
		when(relatedCollectorItemRepository.saveRelatedItems(left, right, "some source", "some reason"))
			.thenReturn(rc2);
		RelatedCollectorItem savedAgain = relatedCollectorItemRepository.saveRelatedItems(left, right, "some source",
				"some reason");

		List<RelatedCollectorItem> relatedCollectorItemList = Lists
			.newArrayList(relatedCollectorItemRepository.findAll());
		assertFalse(CollectionUtils.isEmpty(relatedCollectorItemList));
		assertEquals(1, relatedCollectorItemList.size());
		assertEquals(relatedCollectorItemList.get(0).getLeft(), left);
		assertEquals(relatedCollectorItemList.get(0).getRight(), right);
		assertTrue(relatedCollectorItemList.get(0).getReason().equalsIgnoreCase("some reason"));
		assertTrue(relatedCollectorItemList.get(0).getSource().equalsIgnoreCase("some source"));
		assertNotEquals(saved.getId(), savedAgain.getId());

	}

	@Test
	void saveRelatedItemsDuplicateMany() {
		relatedCollectorItemRepository.deleteAll();
		...
		assertEquals(relatedCollectorItemRepository.findAll().iterator().next(), items.get(0));
		...
		assertEquals(relatedCollectorItemRepository.saveRelatedItems(left, right, "some source", "some reason"),
				savedAgain);
