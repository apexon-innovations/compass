...
import com.apexon.compass.dashboard.util.PipelineUtils;
...
public class EnvironmentComponentEventListener extends HygieiaMongoEventListener<EnvironmentComponent> {
	...
	public EnvironmentComponentEventListener(DashboardRepository dashboardRepository,
			...
			BinaryArtifactRepository binaryArtifactRepository, PipelineRepository pipelineRepository,
			CollectorRepository collectorRepository, BuildRepository buildRepository, JobRepository<?> jobRepository,
	...
	/**
	 * For the environment component, find all team dashboards related to the environment
	 * component and add the commits to the proper stage
	...
	/**
	 * Must first start by finding all artifacts that relate to an environment component
	 * based on the name, and potentially the timestamp of the last time an artifact came
	...
	private void addCommitsToEnvironmentStage(EnvironmentComponent environmentComponent, Pipeline pipeline) {
		EnvironmentStage currentStage = getOrCreateEnvironmentStage(pipeline,
				environmentComponent.getEnvironmentName());
		String pseudoEnvName = environmentComponent.getEnvironmentName();
		if (LOGGER.isDebugEnabled()) {
			LOGGER.debug("Attempting to find new artifacts to process for environment '"
					+ environmentComponent.getEnvironmentName() + "'");
		}
		...
		if (oldLastArtifact != null) {
			...
			artifacts.addAll(Lists.newArrayList(
					binaryArtifactRepository.findByArtifactNameAndArtifactExtensionAndTimestampGreaterThan(artifactName,
							artifactExtension, lastArtifactTimestamp)));

			// Backwards compatibility
			if (artifactExtension != null) {
				// In the past the extension was saved as part of the artifact name
				artifacts.addAll(Lists.newArrayList(
						binaryArtifactRepository.findByArtifactNameAndArtifactExtensionAndTimestampGreaterThan(
								environmentComponent.getComponentName(), null, lastArtifactTimestamp)));
			}
		}
		else {
		...
		// Sort the artifacts by timestamp and iterate through each artifact, getting
		// their changesets and adding them to the bucket
		List<BinaryArtifact> sortedArtifacts = Lists.newArrayList(artifacts);
		if (CollectionUtils.isEmpty(sortedArtifacts))
			return;
		...
		for (BinaryArtifact artifact : sortedArtifacts) {
			if (artifact == null)
				continue;
			if (LOGGER.isDebugEnabled()) {
				LOGGER.debug("Processing artifact " + artifact.getArtifactGroupId() + ":" + artifact.getArtifactName()
			...
			if (CollectionUtils.isEmpty(builds)) {
				// Attempt to get the build based on the artifact metadata information if
				// possible
			...
			/*
			 * If some build events are missed, here is an attempt to move commits to the
			 * build stage This also takes care of the problem with Jenkins first build
			 * change set being empty.
			 *
			 * Logic: If the build start time is after the scm commit, move the commit to
			 * build stage. Match the repo at the very least.
			 */
			Map<String, PipelineCommit> commitStageCommits = pipeline
				.getCommitsByEnvironmentName(PipelineStage.COMMIT.getName());
			...
			for (Map.Entry<String, PipelineCommit> e : commitStageCommits.entrySet()) {
				...
				if ((commit.getScmCommitTimestamp() < build.getStartTime()) && !envStageCommits.containsKey(e.getKey())
						&& PipelineUtils.isMoveCommitToBuild(build, commit, commitRepository)) {
	...
	/**
	 * Attempts to find the build for the artifact based on the artifacts build metadata
	 * information.
	 * @param artifact
	 * @return
	 */
	private Build getBuildByMetadata(BinaryArtifact artifact) {
		...
		// Note: in order to work properly both the artifact and the build must exist when
		// this is run
		// This shouldn't be a problem as they would exist by the time the component is
		// deployed so
		...
		if (jobName != null && buildNumber != null && instanceUrl != null) {
			...
			// Just in case more build collectors are added in the future that run
			// together...
		...
		if (build == null) {
			LOGGER.debug("Artifact " + artifact.getId() + " references build " + buildNumber + " in '" + instanceUrl
					+ "' but no build with that information was found.");
		}
	...
	/**
	 * Finds team dashboards for a given environment componentby way of the deploy
	 * collector item
	 * @param environmentComponent
	 * @return
	 */
	private List<Dashboard> findTeamDashboardsForEnvironmentComponent(EnvironmentComponent environmentComponent) {
		Optional<CollectorItem> optDeploymentCollectorItem = collectorItemRepository
			.findById(environmentComponent.getCollectorItemId());
