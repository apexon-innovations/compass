...
public final class PipelineUtils {
	...
	public static Map<PipelineStage, String> getStageToEnvironmentNameMap(Dashboard dashboard) throws HygieiaException {
		...
		for (Widget widget : dashboard.getWidgets()) {
			...
			if (widget.getName().equalsIgnoreCase("pipeline")) {
				...
				if (gh == null) {
					throw new HygieiaException("Pipeline " + widget.getId() + " is missing mappings ",
	...
	/**
	 * Iterate over failed builds, if the failed build collector item id matches the
	 * successful builds collector item id take all the commits from the changeset of the
	 * failed build and add them to the pipeline. Then remove the failed build from the
	 * collection after it has been processed.
