...
/**
 * Entry class to hold the Auto Discovered entries: Jira project, github project, build
 * job etc.
 */
public class AutoDiscoveredEntry {
	...
	public boolean equals(Object o) {
		...
		if (o == null || !(o instanceof AutoDiscoveredEntry))
			return false;
	...
	@Override
	public int hashCode() {
		return new HashCodeBuilder(17, 37).append(status).append(description).append(options).toHashCode();
	}
	...
	public AutoDiscoverCollectorItem toAutoDiscoverCollectorItem(Collector collector) throws HygieiaException {
		if (options.keySet().containsAll(collector.getUniqueFields().keySet())) {
			...
			throw new HygieiaException(
					"Missing required fields. " + toolName + " collector required fields are: "
							+ String.join(", ", collector.getUniqueFields().keySet()),
