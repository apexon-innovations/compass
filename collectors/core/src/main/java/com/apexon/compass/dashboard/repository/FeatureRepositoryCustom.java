...
public interface FeatureRepositoryCustom {
	...
	/**
	 * Find all features with active sprints that are between the provided date and that
	 * have an end date < 9999-12-31 EOD
	 * @param sTeamId
	 * @param sProjectId
	 ...
	 * @return
	 */
	List<Feature> findByActiveEndingSprints(String sTeamId, String sProjectId, ObjectId collectorId,
			String currentISODateTime, boolean minimal);

	/**
	 * Find all features that have sprints set but without an end date (or an end date >=
	 * 9999-12-31 EOD)
	 * @param sTeamId
	 * @param sProjectId
	 * @param minimal
