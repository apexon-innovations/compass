 ...
 */
public interface CollectorItemRepository
		extends BaseCollectorItemRepository<CollectorItem>, QuerydslPredicateExecutor<CollectorItem> {

	//// FIXME: 1/20/16 I really hate this dashboard specific method in the collectoritem
	//// repository, should we move the dashboardcollectoritem repository into core?
	...
	// FIXME: 3/1/16 Really need to refactor this. Do not want collector specific lookups
	// here.
	...
	@Query(value = "{ 'collectorId' : ?0, 'options.url' : {$regex : '^?1$', $options: 'i'}, 'options.branch' : {$regex : '^?2$', $options: 'i'}, enabled : ?3}")
	List<CollectorItem> findAllRepoByUrlAndBranchAndEnabled(ObjectId collectorId, String url, String branch,
			boolean enabled);
	...
	Page<CollectorItem> findByCollectorIdAndSearchField(List<ObjectId> collectorId, String searchField,
			String searchFieldValue, Pageable pageable);

	@Query(value = "{ 'collectorId': { $in: ?0 }, ?1 : {$regex : '.*?2.*', $options: 'i'} ,  ?3 : {$regex : '.*?4.*', $options: 'i'}}")
	Page<CollectorItem> findByCollectorIdAndSearchFields(List<ObjectId> collectorId, String searchField1,
			String searchFieldValue1, String searchField2, String searchFieldValue2, Pageable pageable);
	...
	CollectorItem findByArtifactNameAndArtifactVersionAndTestResultsUrl(String artifactName, String artifactVersion,
	...
	default Iterable<CollectorItem> findAllByOptionNameValueAndCollectorIdsIn(String optionName, String optionValue,
			List<ObjectId> collectorIds) {
		Map<String, Object> inMap = new HashMap<>();
		inMap.put(optionName, optionValue);
		return findAllByOptionMapAndCollectorIdsIn(inMap, collectorIds);
	}

	default Iterable<CollectorItem> findAllByOptionMapAndCollectorIdsIn(Map<String, Object> options,
			List<ObjectId> collectorIds) {
		PathBuilder<CollectorItem> path = new PathBuilder<>(CollectorItem.class, "collectorItem");
		BooleanBuilder builder = new BooleanBuilder();
		builder.and(path.get("collectorId", ObjectId.class).in(collectorIds));
		options.forEach((key, value) -> builder
			.and(Objects.isNull(value) ? path.get("options", Map.class).get(key, Object.class).isNull()
					: path.get("options", Map.class).get(key, Object.class).eq(value)));
		return findAll(builder.getValue());
	}
