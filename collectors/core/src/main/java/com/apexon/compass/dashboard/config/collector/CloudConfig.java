package com.apexon.compass.dashboard.config.collector;

import com.apexon.compass.dashboard.model.CollectorItem;
import com.apexon.compass.dashboard.model.NameValue;
...
public class CloudConfig extends CollectorItem {
	...
	public double getCpuError() {
		return (getOptions().get(CPU_ERROR) != null) ? (double) getOptions().get(CPU_ERROR)
				: CPU_ERROR_THRESHOLD_DEFAULT;
	}

	public double getCpuAlert() {
		return (getOptions().get(CPU_ALERT) != null) ? (double) getOptions().get(CPU_ALERT)
				: CPU_ALERT_THRESHOLD_DEFAULT;
	}

	public double getMemoryError() {
		return (getOptions().get(MEMORY_ERROR) != null) ? (long) getOptions().get(MEMORY_ERROR)
				: MEMORY_ERROR_THRESHOLD_DEFAULT;
	}

	public double getMemoryAlert() {
		return (getOptions().get(MEMORY_ALERT) != null) ? (long) getOptions().get(MEMORY_ALERT)
				: MEMORY_ALERT_THRESHOLD_DEFAULT;

	}

	public long getDiskioError() {
		return (getOptions().get(DISKIO_ERROR) != null) ? (long) getOptions().get(DISKIO_ERROR)
				: DISK_IO_ERROR_THRESHOLD_DEFAULT;
	}

	public long getDiskioAlert() {
		return (getOptions().get(DISKIO_ALERT) != null) ? (long) getOptions().get(DISKIO_ALERT)
				: DISK_IO_ALERT_THRESHOLD_DEFAULT;
	}

	public long getNetworkioError() {
		return (getOptions().get(NETWORKIO_ERROR) != null) ? (long) getOptions().get(NETWORKIO_ERROR)
				: NETWORK_IO_ERROR_THRESHOLD_DEFAULT;
	}

	public long getNetworkioAlert() {
		return (getOptions().get(NETWORKIO_ALERT) != null) ? (long) getOptions().get(NETWORKIO_ALERT)
				: NETWORK_IO_ALERT_THRESHOLD_DEFAULT;
	}
