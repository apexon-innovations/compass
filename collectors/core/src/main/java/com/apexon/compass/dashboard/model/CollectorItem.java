...
/**
 * <p>
 * Represents a unique collection in an external tool. For example, for a CI tool the
 * collector item would be a Job. For a project management tool, the collector item might
 * be a Scope.
 * </p>
 * <p>
 * Each {@link Collector} is responsible for specifying how it's {@link CollectorItem}s
 * are uniquely identified by storing key/value pairs in the options Map. The description
 * field will be visible to users in the UI to aid in selecting the correct
 * {@link CollectorItem} for their dashboard. Ideally, the description will be unique for
 * a given {@link Collector}.
 * </p>
 */
...
public class CollectorItem extends BaseModel {
	...
	public boolean checkErrorOrReset(long resetWindow, int errorThreshold) {
		Optional<CollectionError> lastErrorOptional = errors.stream()
			.max(Comparator.comparingLong(CollectionError::getTimestamp));
		long lastErrorTimestamp = lastErrorOptional.isPresent() ? lastErrorOptional.get().getTimestamp()
