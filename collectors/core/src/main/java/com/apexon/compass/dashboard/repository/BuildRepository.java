...
import com.apexon.compass.dashboard.model.BuildStatus;
...
public interface BuildRepository extends CrudRepository<Build, ObjectId>, QuerydslPredicateExecutor<Build> {
	...
	List<Build> findBuildsForRevisionNumbersAndBuildCollectorItemIds(List<String> scmRevisionNumbers,
			List<ObjectId> buildCollectorItemId);

	Build findTop1ByCollectorItemIdOrderByTimestampDesc(ObjectId collectorItemId);

	Build findTop1ByCollectorItemIdAndBuildStatusOrderByTimestampDesc(ObjectId collectorItemId,
			BuildStatus buildStatus);

	Build findTop1ByCollectorItemIdAndTimestampIsBetweenOrderByTimestampDesc(ObjectId collectorItemId, long beginDate,
			long endDate);
