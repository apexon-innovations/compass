...
public class CollectorServiceImpl implements CollectorService {
	...
	@Override
	public Page<CollectorItem> collectorItemsByTypeWithFilter(CollectorType collectorType, String searchFilterValue,
			Pageable pageable) {
		...
		if (!StringUtils.isEmpty(searchFilter.getAdvancedSearchKey()) && criteria.size() > 1) {
			...
			collectorItems = collectorItemRepository.findByCollectorIdAndSearchFields(collectorIds, defaultSearchField,
					searchFilter.getSearchKey(), advSearchField, searchFilter.getAdvancedSearchKey(), pageable);
		}
		else {
			// single search criteria
			collectorItems = collectorItemRepository.findByCollectorIdAndSearchField(collectorIds, defaultSearchField,
					searchFilterValue, pageable);
		}
	...
	@Override
	public Page<CollectorItem> collectorItemsByTypeWithFilter(CollectorType collectorType, String searchFilterValue,
			String searchField, Pageable pageable) {
		List<Collector> collectors = collectorRepository.findByCollectorType(collectorType);
		List<ObjectId> collectorIds = Lists.newArrayList(Iterables.transform(collectors, new ToCollectorId()));
		Page<CollectorItem> collectorItems;
		collectorItems = collectorItemRepository.findByCollectorIdAndSearchField(collectorIds, searchField,
				searchFilterValue, pageable);
		removeJobUrlAndInstanceUrl(collectorItems);
		for (CollectorItem options : collectorItems) {
			options.setCollector(collectorById(options.getCollectorId(), collectors));
	...
	/**
	 * We want to initialize the Quasi-product collector when the API starts up so that
	 * any existing Team dashboards will be added as CollectorItems.
	 * <p>
	...
	private List<CollectorItem> lookUpCollectorItem(CollectorItem collectorItem) {
		...
		Map<String, Object> uniqueOptions = collector.getUniqueFields()
			...
			.collect(Collectors.toMap(java.util.function.Function.identity(),
					option -> collectorItem.getOptions().get(option), (a, b) -> a));
		if (MapUtils.isEmpty(uniqueOptions)) {
			return Collections.emptyList();
		}
		return IterableUtils.toList(collectorItemRepository.findAllByOptionMapAndCollectorIdsIn(uniqueOptions,
				Lists.newArrayList(collector.getId())));

	}

	// This is to handle scenarios where the option contains user credentials etc. We do
	// not want to create a new collector item -
	...
	public CollectorItem createCollectorItemSelectOptions(CollectorItem item, Map<String, Object> allOptions,
			Map<String, Object> uniqueOptions) {
	...
	@Override
	public CollectorItem createCollectorItemSelectOptions(CollectorItem item, Collector collector,
			Map<String, Object> allOptions, Map<String, Object> uniqueOptions) {
		Map<String, Object> uniqueFieldsFromCollector = collector.getUniqueFields();
		List<CollectorItem> existing = customRepositoryQuery.findCollectorItemsBySubsetOptions(item.getCollectorId(),
				allOptions, uniqueOptions, uniqueFieldsFromCollector);
	...
	public CollectorItem createCollectorItemByNiceNameAndProjectId(CollectorItem item, String projectId)
		...
		// if not found, call the method to look up by collector ID and options. NiceName
		// would be saved too
		if (existing == null)
			return createCollectorItem(item);

		// Flow is here because there is only one collector item with the same collector
		// id and niceName. So, update with
	...
	public List<CollectorItem> getCollectorItemForComponent(String id, String type) {
		...
		// the collector items from component are not updated for collector run. We need
		// to
	...
	public void deleteCollectorItem(String id, boolean deleteFromComponent) throws HygieiaException {
		...
		if (deleteFromComponent) {
			List<Component> components = componentRepository.findByCollectorTypeAndItemIdIn(type,
	...
	private List<String> getSearchFields(List<Collector> collectors) {
		List<List<String>> searchList = Lists
			.newArrayList(Iterables.transform(collectors, new ToCollectorSearchFields()));
		return (!searchList.isEmpty() && searchList.get(0) != null)
				? searchList.stream().flatMap(List::stream).collect(Collectors.toList()) : null;
	}
		...
		if (StringUtils.isNotEmpty(collectorName) && StringUtils.isNotEmpty(projectName)) {
			...
			if (Objects.nonNull(collector)) {
				Iterable<CollectorItem> collectorItems = collectorItemRepository
					.findAllByOptionNameValueAndCollectorIdsIn("projectName", projectName,
							Arrays.asList(collector.getId()));
				if (!IterableUtils.isEmpty(collectorItems)) {
					List<ObjectId> collectorItemIds = IterableUtils.toList(collectorItems)
						.stream()
						.map(CollectorItem::getId)
						.collect(Collectors.toList());
					List<Component> components = componentRepository
						.findByCollectorTypeAndItemIdIn(CollectorType.CodeQuality, collectorItemIds);
					if (CollectionUtils.isNotEmpty(components)) {
						List<ObjectId> componentIds = components.stream()
							.map(Component::getId)
							.collect(Collectors.toList());
						...
						if (CollectionUtils.isNotEmpty(dashboards)) {
							Set<Cmdb> cmdbs = dashboards.stream()
								.map(dashboard -> cmdbRepository.findByConfigurationItemAndItemTypeAndValidConfigItem(
										dashboard.getConfigurationItemBusAppName(), "component", true))
