...
import com.apexon.compass.dashboard.model.EnvironmentComponent;
...
public interface EnvironmentComponentRepository extends CrudRepository<EnvironmentComponent, ObjectId> {
	...
	/**
	 * Finds the {@link EnvironmentComponent} collector item id, environment name and
	 * component name.
	...
	/**
	 * Finds all {@link EnvironmentComponent}s for a given
	...
	List<EnvironmentComponent> findDeployedByCollectorItemIdAndComponentNameAndComponentVersion(
			ObjectId dashboardCollectorItemId, String componentName, String componentVersion);

	@Query(value = "{ 'collectorItemId': ?0, 'componentName': ?1, 'componentVersion': ?2, 'deployTime':?3}")
	EnvironmentComponent findByUniqueKey(ObjectId collectorItemId, String componentName, String componentVersion,
			long deployTime);
