...
import static com.apexon.compass.dashboard.util.PipelineUtils.isMoveCommitToBuild;
import static com.apexon.compass.dashboard.util.PipelineUtils.processPreviousFailedBuilds;
...
public class BuildEventListener extends HygieiaMongoEventListener<Build> {
	...
	@Autowired
	public BuildEventListener(DashboardRepository dashboardRepository, CollectorItemRepository collectorItemRepository,
			ComponentRepository componentRepository, PipelineRepository pipelineRepository,
	...
	/**
	 * If the build has failed, find the pipelines of the dashboards referencing the build
	 * and add the failed build to the failed builds bucket on the pipeline
	 * @param failedBuild
	 */
	...
	/**
	 * Find all dashboards referencing the build and then then for each commit in the
	 * changeset of the build (as per jenkins) add the commit to the pipeline for the
	 * dashboard
	 * @param build
	 */
	private void processBuild(Build build) {
		...
		// for every team dashboard referencing the build, find the pipeline, put this
		// commit in the build stage
		for (Dashboard teamDashboard : teamDashboardsReferencingBuild) {
			List<Component> components = teamDashboard.getApplication().getComponents();
			if (CollectionUtils.isEmpty(components)) {
				continue;
			...
			for (CollectorItem productCI : productCIs) {
				...
				for (SCM scm : build.getSourceChangeSet()) {
					// we want to use the build start time since the timestamp was just
					// the time that the collector ran
				...
				/*
				 * If some build events are missed, here is an attempt to move commits to
				 * the build stage This also takes care of the problem with Jenkins first
				 * build change set being empty.
				 *
				 * Logic: If the build start time is after the scm commit, move the commit
				 * to build stage. Match the repo at the very least.
				 */
				Map<String, PipelineCommit> commitStageCommits = pipeline
					.getCommitsByEnvironmentName(PipelineStage.COMMIT.getName());
				Map<String, PipelineCommit> buildStageCommits = pipeline
					.getCommitsByEnvironmentName(PipelineStage.BUILD.getName());
				for (Map.Entry<String, PipelineCommit> e : commitStageCommits.entrySet()) {
					PipelineCommit commit = e.getValue();
					if ((commit.getScmCommitTimestamp() < build.getStartTime())
							&& !buildStageCommits.containsKey(e.getKey())
							&& isMoveCommitToBuild(build, commit, commitRepository)) {
						pipeline.addCommit(PipelineStage.BUILD.getName(), commit);
					}
				}
				pipelineRepository.save(pipeline);
	...
	/**
	 * Finds all of the dashboards for a given build way of the build by: 1. Get collector
	 * item id for the build 2. Get the build components referencing the build
	 * collectoritem 3. Find all dashboards whose application references the build
	 * components
	 * @param build
	 * @return
	 */
	private List<Dashboard> findAllDashboardsForBuild(Build build) {
		...
		if (buildCollectorItem != null) {
			List<Component> components = componentRepository
				.findByBuildCollectorItemId(buildCollectorItem.get().getId());
