...
import com.apexon.compass.dashboard.repository.PerformanceRepository;
...
public class TestResultEventListener extends AbstractMongoEventListener<TestResult> {
	...
	private enum PERFORMANCE_METRICS {
		...
		averageResponseTime, totalCalls, errorsperMinute, actualErrorRate, businessTransactionHealthPercent,
		nodeHealthPercent, violationObject, totalErrors, errorRateSeverity, responseTimeSeverity, callsperMinute,
		targetResponseTime, targetTransactionPerSec, targetErrorRateThreshold
	...
	@Autowired
	public TestResultEventListener(CollectorRepository collectorRepository,
			CollectorItemRepository collectorItemRepository, PerformanceRepository performanceRepository) {
	...
	public void onAfterSave(AfterSaveEvent<TestResult> event) {
		...
		lastExecutedTestCapabiblity.getTestSuites()
			.forEach(testSuite -> testSuite.getTestCases()
				.forEach(testCase -> extractPerformanceMetrics(testCase, lastExecutedTestCapabiblity)));
	 ...
	 */
	public Performance createPerformanceDoc(TestResult testResult, TestCapability testCapability,
			CollectorItem perfCollectorItem) {
	...
	public CollectorItem getPerfCollectorItem(TestResult testResult) {
		...
		Optional<CollectorItem> optTestResultCollItem = collectorItemRepository
			.findById(testResult.getCollectorItemId());
		...
		Optional<CollectorItem> optCollectorItem = Optional.ofNullable(collectorItemRepository
			.findByCollectorIdNiceNameAndJobName(perfToolsCollector.getId(), niceName, jobName));
	...
	public LinkedHashMap<String, Object> getPerfMetrics(TestCapability testCapability) {
		...
		// If test execution duration less than a minute, calculate calls per minute from
		// actual test execution seconds
		double callsPerMinuteVal = testCapabilitySecs > SIXTY_SECS ? actualTxnsPerSec * SIXTY_SECS
				: actualTxnsPerSec * testCapabilitySecs;
		...
		double errorsPerMinuteVal = testCapabilitySecs > SIXTY_SECS ? actualErrorsPerSec * SIXTY_SECS
				: actualErrorsPerSec * testCapabilitySecs;
