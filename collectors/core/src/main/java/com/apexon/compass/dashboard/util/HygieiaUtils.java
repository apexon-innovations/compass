...
public class HygieiaUtils {
	...
	public static void mergeObjects(Object dest, Object source)
	...
	/**
	 * Determines if two urls are equal accounting for load balancers and variations in
	 * schemes.
	 * <p>
	 ...
	 * @param url1
	 * @param url2
	 * @return if the two urls are equal ignoring load balancers, url schemes, and path
	 * endings.
	 */
	...
	public static boolean smartUrlEquals(String url1, String url2) {
		...
		return safeEquals(u1PrimaryDomain, u2PrimaryDomain) && safeEquals(u1Path, u2Path)
	...
	/*
	 * Returns true if any of the input String parameters is empty. Else, returns false.
	 */
	...
	public static boolean allowSync(FeatureFlag featureFlag, CollectorType collectorType) {
		...
		if (MapUtils.isEmpty(featureFlag.getFlags()) || Objects.isNull(featureFlag.getFlags().get(key)))
			return true;
		return !BooleanUtils.toBoolean(featureFlag.getFlags().get(StringUtils.lowerCase(collectorType.toString())));
	}

	/*
	 * If Feature flag is present then check for the collectortype and see if its enabled
	 * for AutoDiscover.
	 */
	...
	/*
	 * normalize build url by removing build number Eg: Input :
	 * https://jenkins.com/job/test/job/youseeme/job/Reference/job/master/228/ Output :
	 * https://jenkins.com/job/test/job/youseeme/job/Reference/job/master/
	 *
	 */
