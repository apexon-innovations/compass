...
public class LibraryPolicyResult extends BaseModel {
	...
	public void addThreat(LibraryPolicyType type, LibraryPolicyThreatLevel level,
			LibraryPolicyThreatDisposition disposition, String dispositionStatus, String component, String age,
			String score) {
		Set<Threat> threatSet = threats.get(type);

		if (CollectionUtils.isEmpty(threatSet)) {
			Threat threat = new Threat(level, 1);
			threat.getComponents()
				.add(getComponentPlusDispositionPlusAge(component, disposition, dispositionStatus, age, score));
			...
			for (Threat t : threatSet) {
				if (Objects.equals(t.getLevel(), level)) {
					...
					if (!t.getComponents()
						.contains(getComponentPlusDispositionPlusAge(component, disposition, dispositionStatus, age,
								score))) {
						t.getComponents()
							.add(getComponentPlusDispositionPlusAge(component, disposition, dispositionStatus, age,
									score));
					}
					threatSet.add(t);
					threats.put(type, threatSet);
	...
	public void addThreat(LibraryPolicyType type, LibraryPolicyThreatLevel level,
			LibraryPolicyThreatDisposition disposition, String dispositionStatus, String component, String age,
			String score, String policyName) {
		Set<Threat> threatSet = threats.get(type);

		if (CollectionUtils.isEmpty(threatSet)) {
			Threat threat = new Threat(level, 1);
			threat.getComponents()
				.add(getComponentPlusDispositionPlusAgePlusPolicyName(component, disposition, dispositionStatus, age,
						score, policyName));
			threat.addDispositionCount(disposition);
			setCriticalAndHighVulAge(age, threat, disposition);
			setCriticalAndHighVulScore(score, threat, disposition);
			...
			for (Threat t : threatSet) {
				if (Objects.equals(t.getLevel(), level)) {
					...
					setCriticalAndHighVulScore(score, t, disposition);
					if (!t.getComponents()
						.contains(getComponentPlusDispositionPlusAgePlusPolicyName(component, disposition,
								dispositionStatus, age, score, policyName))) {
						t.getComponents()
							.add(getComponentPlusDispositionPlusAgePlusPolicyName(component, disposition,
									dispositionStatus, age, score, policyName));
					}
					threatSet.add(t);
					threats.put(type, threatSet);
			...
			if (!found) {
				...
				if (!t.getComponents()
					.contains(getComponentPlusDispositionPlusAgePlusPolicyName(component, disposition,
							dispositionStatus, age, score, policyName))) {
					t.getComponents()
						.add(getComponentPlusDispositionPlusAgePlusPolicyName(component, disposition, dispositionStatus,
								age, score, policyName));
				}
				t.addDispositionCount(disposition);
				setCriticalAndHighVulAge(age, t, disposition);
				setCriticalAndHighVulScore(score, t, disposition);
	...
	private String getComponentPlusDispositionPlusAge(String component, LibraryPolicyThreatDisposition disposition,
			String dispositionStatus, String age, String score) {
		return String.format("%s##%s##%s##%s##%s", component, disposition.toString(), age, dispositionStatus, score);
	}

	private String getComponentPlusDispositionPlusAgePlusPolicyName(String component,
			LibraryPolicyThreatDisposition disposition, String dispositionStatus, String age, String score,
			String policyName) {
		return String.format("%s##%s##%s##%s##%s##%s", component, disposition.toString(), age, dispositionStatus, score,
				policyName);
	}
